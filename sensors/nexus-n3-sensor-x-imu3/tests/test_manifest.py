...
def test_plugin_manifest_exists() -> None:
    ...
    packaged_manifest = json.loads((plugin_root / "src" / "nexus_n3_sensor_x_imu3" / "plugin.json").read_text(encoding="utf-8"))
    assert packaged_manifest == root_manifest
    assert root_manifest["plugin_type"] == "sensor"
    assert root_manifest["entry_point"] == "nexus_n3_sensor_x_imu3.sensor:XImu3Sensor"
